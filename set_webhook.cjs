...
async function setWebhook() {
    console.log("DISABLE Webhook for 'vendas'...");
    ...
    await supabase.functions.invoke('evolution-manager', {
        body: {
            action: 'set-webhook',
            instanceName: 'vendas',
            webhookUrl: 'https://ahvaqriovmsxixgilkxa.supabase.co/functions/v1/webhook-whatsapp-DISABLED' // Dummy URL
        }
    });

    // 2. Enable with clean config
    await new Promise(r => setTimeout(r, 2000));
    console.log("ENABLE Webhook for 'vendas'...");
